...
import { Colors, ICON_SIZE, PADDING, SEGMENT } from "../commons/defined";
...
const styles = StyleSheet.create({
 ...
 const TabBarNavigationParticules = ({ transition, activeTransition }: PariculesProps) => {
    const middle = HEIGHT / 2 - size / 2;
    const x = add(multiply(transition, SEGMENT), SEGMENT / 2 - size / 2);
    const top = interpolateNode(activeTransition, {
        inputRange: [0, 0.5, 1],
        outputRange: [middle, PADDING / 2, middle],
    });
    const bottom = interpolateNode(activeTransition, {
        inputRange: [0, 0.5, 1],
        outputRange: [middle, HEIGHT - PADDING / 2, middle],
    });
    const s = interpolateNode(activeTransition, {
        inputRange: [0, 0.5, 1],
 ...
 export default TabBarNavigationParticules;
